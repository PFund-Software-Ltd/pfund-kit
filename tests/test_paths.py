...
def mock_platformdirs(tmp_path, monkeypatch):
    ...
    still index returned dict by 'log'/'data'/'cache'/'config' — those now point
    at the per-project layout root, NOT at OS-native dirs.
    """
    monkeypatch.setenv('HOME', str(tmp_path))
    monkeypatch.setenv('USERPROFILE', str(tmp_path))  # Windows equivalent
    # Note: for the new layout, log/data/cache/config all live under the same
    # ~/.{project_name}/ root, so these keys are effectively aliases for tmp_path.
    return {
        'log': tmp_path,
        'data': tmp_path,
        'cache': tmp_path,
