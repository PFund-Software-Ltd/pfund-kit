...
class RichStyle:
    ...
    def __add__(self, other):
        """Combine styles: RichTextStyle.BOLD + RichColor.RED -> 'bold red'"""
        if isinstance(other, (RichStyle, RichColor, RichTextStyle)):
            return RichStyle(f"{self._value} {other._value if hasattr(other, '_value') else other.value}")
        elif isinstance(other, str):
            return RichStyle(f"{self._value} {other}")
        return NotImplemented
    
    def __radd__(self, other):
        """Support reverse addition: RichColor.RED + RichTextStyle.BOLD"""
        if isinstance(other, str):
            return RichStyle(f"{other} {self._value}")
        return NotImplemented
    ...
    def __repr__(self):
        return f"RichStyle('{self._value}')"
...
class RichTextStyle:
    ...
    These can be combined using + operator:
        RichTextStyle.BOLD + RichColor.RED
        RichTextStyle.BOLD + RichTextStyle.UNDERLINE + RichColor.CYAN
    """
    BOLD = RichStyle("bold")
    DIM = RichStyle("dim")
    ITALIC = RichStyle("italic")
    UNDERLINE = RichStyle("underline")
    UNDERLINE2 = RichStyle("underline2")
    BLINK = RichStyle("blink")
    BLINK2 = RichStyle("blink2")
    REVERSE = RichStyle("reverse")
    CONCEAL = RichStyle("conceal")
    STRIKE = RichStyle("strike")
    OVERLINE = RichStyle("overline")


class RichColor(StrEnum):
    """All available Rich named colors.
    
    Can be used directly or combined with RichTextStyle:
        bar_style=RichColor.GREEN
        text_style=RichTextStyle.BOLD + RichColor.CYAN
